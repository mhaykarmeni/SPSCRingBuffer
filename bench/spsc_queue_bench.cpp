...
static void BM_PushOnly_Mtx(benchmark::State& state) {
    SPSCQueueMtx<int, 1024> q;
    int i = 0;
    for (auto _ : state) {
        if (!q.try_push(i++))
            q.try_pop();
...
BENCHMARK(BM_PushOnly_Mtx);

static void BM_PopOnly_Mtx(benchmark::State& state) {
    SPSCQueueMtx<int, 1024> q;
    for (int i = 0; i < 1024; ++i) q.try_push(i);
    for (auto _ : state) {
        if (!q.try_pop())
...
BENCHMARK(BM_PopOnly_Mtx);

static void BM_Throughput_Mtx(benchmark::State& state) {
    SPSCQueueMtx<int, 4096> q;
    constexpr int BATCH = 100'000;
...
BENCHMARK(BM_Throughput_Mtx)->Unit(benchmark::kMillisecond);

static void BM_Latency_RTT_Mtx(benchmark::State& state) {
    SPSCQueueMtx<int, 4096> q_req;
    SPSCQueueMtx<int, 4096> q_resp;
...
BENCHMARK(BM_Latency_RTT_Mtx)->Unit(benchmark::kNanosecond);
